const read_text_file = (function () {
  ...
  let weekday = [
    1, 1, 1, 1, 1, 3, 8, 20, 10, 15, 15, 20, 25, 12, 15, 20, 18, 16, 10, 8, 8, 7, 4, 2,
  ];
  ...
  return function read_text_file(filename, callback) {
    if (/^activity/.test(filename)) generate_logs();
...
async function bad_file_sizes(files) {
  let list = '';
  await Promise.all(
...
async function file_sizes(files) {
  let lines = await Promise.all(
    files.map(async (filename) => {
      return filename + ': ' + (await text_file(filename)).length;
    }),
  );
...
// bad_file_sizes(["camera_logs.txt", "shopping_list.txt"]).then(console.log)
// file_sizes(["camera_logs.txt", "shopping_list.txt"]).then(console.log)
