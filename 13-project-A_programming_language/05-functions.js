...
special_forms.fun = (args, scope) => {
  ...
  if (!args_length) throw new SyntaxError('Functions need a body');
  ...
  return function (...args) {
    ...
    if (args_length != params.length) throw new TypeError('Wrong number of arguments');

    let local_scope = Object.create(scope);

    for (let i = 0; i < args_length; i++) local_scope[params[i]] = args[i];
