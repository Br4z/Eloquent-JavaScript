...
export function find_route(graph, from, to) {
  // Always found a route, because all the places are connected
  ...
  for (let i = 0; i < work.length; i++) {
    ...
    for (let place of graph[at]) {
      ...
      if (!work.some((w) => w.at == place))
        // Avoid exploring the same path twice
...
export function goal_oriented_robot({ place, parcels }, route) {
  if (route.length == 0) {
    ...
    if (parcel.place != place)
      // Pick up selected parcel
      route = find_route(road_graph, place, parcel.place); // Deliver selected package
    else route = find_route(road_graph, place, parcel.address);
  }
